...
# - Tests may verify readable Sonos log hints using expect_log_contains / expect_log_not_contains.
# - The runtime host/IP and web server port are detected from LoxBerry.
# - Playlist favorite one-click actions get longer internal timeouts.
# - Info diagnostics with network/device calls get action-specific internal timeouts.
# - One-click actions get action-specific internal timeouts.
# - External provider actions get action-specific internal timeouts.
# - Device actions get action-specific internal timeouts.
# - Alarm actions get action-specific internal timeouts.
...
#     "url": "action=...&zone=...",
#     "expect_http": 200,
#     "expect_log_contains": "Unknown URL parameter 'volum'",
#     "expect_log_hint": "The typo was detected and a useful hint was logged.",
#     "timeout_sec": 1,
#     "risk": "safe",
#     "category": "status"
#   }
...
# Version: REGRESSION_TEST_LOG_EXPECTATIONS_V24_2026_06_19
# - Adds text-based Sonos log expectations for readable warnings and diagnostics.
# - Adds expect_log_contains, expect_log_not_contains and expect_log_hint JSON fields.
# - Replaces RAUM and RAUM1 placeholders globally before executing a test URL.
# - RAUM1 is replaced by the selected member override, e.g. host=RAUM1.
# - RAUM is replaced by the selected zone override.
# Relocated from tests/regression_test.pl to src/Support/Testing/regression_test.pl.
# Original functional version marker below:
# REGRESSION_TEST_V20_2026_06_08_FOLLOW_ACTION_TIMEOUTS
# V15:
# - Prefer LoxBerry::System::get_localip() for the runtime base URL.
# - This avoids DNS/hosts issues when lbhostname() is not resolvable from the LoxBerry itself.
# - lbhostname() is only used as fallback if no local IP can be detected.
#
# V14:
...
my $SCRIPT_VERSION       = "REGRESSION_TEST_V24_2026_06_19_LOG_EXPECTATIONS";
my $DEFAULT_BASE_DIR     = "REPLACELBHOMEDIR/config/plugins/sonos4lox";
my $DEFAULT_JSON_FILE    = "$DEFAULT_BASE_DIR/url_tests.json";
my $DEFAULT_TEST_LOG     = "REPLACELBHOMEDIR/log/plugins/sonos4lox/regression_test.log";
my $SONOS_LOG_FILE       = "REPLACELBHOMEDIR/log/plugins/sonos4lox/sonos.log";
my $DEFAULT_TEST_ENDPOINT_PATH = "/plugins/sonos4lox/index.php/";
my $SOURCE_PLACEHOLDER = "SOURCE";
my $ZONE_PLACEHOLDER   = "RAUM";
my $MEMBER_PLACEHOLDER = "RAUM1";
...
foreach my $test (@{$tests}) {
    ...
    my @expect_log_contains  = normalize_expected_text_list($test->{expect_log_contains});
    my @expect_log_not_contains = normalize_expected_text_list($test->{expect_log_not_contains});
    my $expect_log_hint      = rt_trim(safe_value($test->{expect_log_hint}, ""));
    ...
    if (url_contains_runtime_placeholder($url, $MEMBER_PLACEHOLDER)
        && normalize_member_override($selected_members, $selected_zone) eq "") {
        $count_skipped++;
        log_msg("WARNING", "SKIPPED test #$test_number '$name' - URL contains $MEMBER_PLACEHOLDER but no member override was supplied.");
        next;
    }
    ...
    my $result = run_get_test(
        ...
        expect_log_not_contains => \@expect_log_not_contains,
        expect_log_hint         => $expect_log_hint,
    );
    ...
    if ($result->{ok}) {
        ...
        if ($result->{log_expectation_note} ne "") {
            log_msg(
                "OK",
                "OK test #$test_number '$name' - expected Sonos log text found: $result->{log_expectation_note}"
            );
        } elsif ($verbose) {
            log_msg(
                "OK",
...
Runtime placeholders:
  RAUM is replaced by --zone and RAUM1 is replaced by --member.
  RAUM1 can be used in any parameter, for example host=RAUM1 or member=RAUM1.
...
sub build_runtime_base_url {
    ...
    # Prefer the local LoxBerry IP. The short hostname may work from a browser,
    # but it is not guaranteed to resolve from the LoxBerry Perl runtime itself.
    eval {
    ...
    # Fallback to the LoxBerry hostname only if no local IP could be detected.
    if ($host eq "") {
        eval {
...
sub apply_runtime_url_parameters {
    ...
    # Replace explicit placeholders before generic query-parameter handling.
    # Order matters: RAUM1 must be replaced before RAUM, otherwise RAUM1 could
    # accidentally become "<zone>1".
    $url = replace_runtime_placeholder($url, $MEMBER_PLACEHOLDER, $member) if $member ne "";
    $url = replace_runtime_placeholder($url, $ZONE_PLACEHOLDER,   $zone)   if $zone ne "";

    # Important:
    # Member must only be added/overridden for test scenarios that already
    # contain a member parameter in url_tests.json. Do not add member to all tests.
...
sub url_contains_runtime_placeholder {
    my ($url, $placeholder) = @_;

    $url         = rt_trim($url // "");
    $placeholder = rt_trim($placeholder // "");

    return 0 if $url eq "" || $placeholder eq "";

    return ($url =~ /(?<![A-Za-z0-9_])\Q$placeholder\E(?![A-Za-z0-9_])/i) ? 1 : 0;
}

sub replace_runtime_placeholder {
    my ($url, $placeholder, $value) = @_;

    $url         = rt_trim($url // "");
    $placeholder = rt_trim($placeholder // "");
    $value       = rt_trim($value // "");

    return $url if $url eq "" || $placeholder eq "" || $value eq "";

    my $encoded_value = uri_escape_utf8($value);
    $url =~ s/(?<![A-Za-z0-9_])\Q$placeholder\E(?![A-Za-z0-9_])/$encoded_value/ig;
...
sub normalize_expected_text_list {
    my ($value) = @_;
    ...
    if (ref($value) eq 'ARRAY') {
        ...
        foreach my $item (@{$value}) {
            my $text = rt_trim(safe_value($item, ""));
            push @items, $text if $text ne "";
        }
        return @items;
    }

    my $text = rt_trim(safe_value($value, ""));
    return $text ne "" ? ($text) : ();
}

sub file_size_or_zero {
    my ($file) = @_;
...
sub calculate_effective_timeout {
    ...
    my %action_min_timeout = (
        ...
        "setledstate"       => 5,
        "createstereopair"  => 20,
        "seperatestereopair" => 20,
        "networkstatus"     => 30,
        "linein"            => 8,
        "battery"           => 10,
        "getautolinkedzones" => 8,
        "getautoplayvolume" => 8,
        "getuseautoplayvolume" => 8,
        "debuginfo"         => 20,
        "update"            => 10,
        "services"          => 8,
        ...
        "playfavorite"       => 15,
        "playallfavorites"   => 25,
        "playtrackfavorites" => 25,
        "playradiofavorites" => 25,
        "playsonosplaylist"  => 25,
        "playplfavorites"    => 25,
    ...
    } elsif ($name =~ /provider|spotify|amazon|apple|napster|track/) {
        $effective_timeout = max_number($effective_timeout, 25);
    } elsif ($name =~ /follow|leave/) {
        $effective_timeout = max_number($effective_timeout, 20);
    } elsif ($name =~ /device|network|battery|debug|service/) {
        $effective_timeout = max_number($effective_timeout, 15);
    } elsif ($name =~ /alarm/) {
        $effective_timeout = max_number($effective_timeout, 20);
    }
...
sub run_get_test {
    ...
    my $expect_log_contains     = $args{expect_log_contains} || [];
    my $expect_log_not_contains = $args{expect_log_not_contains} || [];
    my $expect_log_hint         = safe_value($args{expect_log_hint}, "");

    $ua->timeout($effective_timeout);

    my $start = time();
    my $sonos_log_offset = file_size_or_zero($SONOS_LOG_FILE);
    my $response;
    ...
    # Give the application logger a very small window to flush its last line.
    sleep 0.10;

    my $sonos_log_segment = read_file_from_offset($SONOS_LOG_FILE, $sonos_log_offset);
    my @php_errors = detect_php_errors($body);

    my @problems;
    my @log_notes;
    my $log_expectation_failed = 0;

    foreach my $needle (@{$expect_log_contains}) {
        next if $needle eq "";

        if (index($sonos_log_segment, $needle) < 0) {
            push @problems, "Expected Sonos log text was not found: '$needle'. "
                . ($expect_log_hint ne "" ? $expect_log_hint : "Please check the URL and the related Sonos log warning.");
            $log_expectation_failed = 1;
        } else {
            push @log_notes, $needle;
        }
    }

    foreach my $needle (@{$expect_log_not_contains}) {
        next if $needle eq "";

        if (index($sonos_log_segment, $needle) >= 0) {
            push @problems, "Unexpected Sonos log text was found: '$needle'. "
                . ($expect_log_hint ne "" ? $expect_log_hint : "Please check the URL and the related Sonos log warning.");
            $log_expectation_failed = 1;
        }
    }
    ...
    return {
        ...
        error_class        => classify_error(
                                ...
                                log_expectation_failed => $log_expectation_failed,
                              ),
        test_number        => $test_number,
        name               => $name,
        url                => $url,
        ...
        body_sample        => shorten(clean_text($body), $response_sample_len),
        php_errors         => \@php_errors,
        sonos_log_sample   => shorten(clean_text($sonos_log_segment), $response_sample_len),
        log_expectation_note => join(" | ", @log_notes),
        expect_log_hint    => $expect_log_hint,
        start_epoch        => $start,
        end_epoch          => $end,
    };
...
sub classify_error {
    ...
    if ($args{log_expectation_failed}) {
        return "LOG_EXPECTATION_MISMATCH";
    }
