...
function muellkalender() {
	...
	// URL from Config
	$url = trim((string)($config['VARIOUS']['CALDavMuell'] ?? ''));
	if ($url === '') {
		S4L_Logger::write('Config for waste calendar is empty - aborting.', 4, __FILE__);
		exit(0);
	}

	$checkdebug = strpos($url, "&debug");
	if ($checkdebug !== false) {
		S4L_Logger::write('Please remove &debug from your syntax entry in Sonos4lox configuration!', 3, __FILE__);
		exit;
	}

	$callurl = trim($url . '&debug');
	...
	$dienstRaw = s4lox_addon_fetch_url($callurl, 10);
	$dienst = s4lox_addon_decode_json($dienstRaw);
	if (empty($dienst) || !is_array($dienst)) {
		S4L_Logger::write('Waste calendar: No or invalid JSON received from CalDav URL!', 4, __FILE__);
		exit;
	}
	...
	if ($events !== false) {
		...
		if (preg_match('/[?&]events=$/', $url)) {
			S4L_Logger::write('Please remove &events= from your syntax entry in Sonos4lox configuration or add the events you are looking for!', 3, __FILE__);
			exit;
		}
	}
	...
	if ($events === false) {
		...
		if ($fwDay === null || $summary === null) {
			S4L_Logger::write('Waste calendar: Missing expected keys (fwDay/Summary) in JSON response!', 4, __FILE__);
			exit;
		}
		...
		if (count($muellheute) === 1 && ($Stunden >= 4 && $Stunden < 17)) {
			...
			S4L_Logger::write('Waste calendar: ' . $text, 6, __FILE__);
			exit;
		}
	}
	...
	S4L_Logger::write('Waste calendar Announcement: ' . $speak, 7, __FILE__);
