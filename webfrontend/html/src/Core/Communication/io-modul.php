<?php
/**
 * Sonos4Lox IO module helper
 * Version: IO_MODULE_CORE_COMMUNICATION_RELOCATION_V01_2026_06_13
 *
 * Purpose:
 * - Provide legacy Loxone UDP/HTTP communication helper functions.
 * - Keep public function names unchanged for URL/runtime compatibility.
 * - Use the central Sonos4Lox logger with src/Core/Communication/io-modul.php context.
 */

require_once dirname(__DIR__, 2) . '/Support/Logger.php';

if (!function_exists('s4lox_io_module_log')) {
    function s4lox_io_module_log($message, $level = S4L_Logger::LEVEL_ERROR)
    {
        S4L_Logger::write((string)$message, (int)$level, __FILE__);
    }
}
...
function udp_send_mem($msnr, $udpport, $prefix, $params)
{
	...
	if(empty($udpport) || $udpport > 65535) {
		s4lox_io_module_log("UDP port $udpport invalid or not defined");
		return 0;
	}
	// lade file aus RAM
...
function ms_udp_send($msnr, $udpport, $prefix, $params)
{
	...
	if (!isset($ms[$msnr])) {
		s4lox_io_module_log("Miniserver $msnr not defined");
		return 0;
	}
	if (!empty($prefix)) {
		$prefix = "$prefix: ";
	...
	if (!isset($udpsocket)) {
		...
		if($udpsocket == NULL) {
			s4lox_io_module_log("Could not create udp socket: " . socket_last_error($udpsocket));
			return 0;
		}
	}
	...
	foreach ($params as $param => $value) {
		...
		if ($parinline == 1 && $currlen > 220) {
			...
			s4lox_io_module_log("msudp_send: Line with one parameter is too long. Parameter $param Value $value skipped.");
			$line = $oldline;
			$udperror = 1;
			continue;
		}
...
function rest_call($msnr, $command) 
{
	...
	if ($xmlresp === false) {
		...
		s4lox_io_module_log("rest_call: An error occured loading the XML:");
		foreach($errors as $error) {
			s4lox_io_module_log(display_xml_error($error, $xmlresp));
		}
...
function http_send($msnr, $inputs, $value = null)
{
	...
	if(!is_array($inputs)) {
		if($value === null) {
			s4lox_io_module_log("http_send: Input string provided, but value missing");
			return;
		}
		// echo "Input is flat\n";
		$inputs = [ $inputs => $value ];
...
function ms_send_mem($msnr, $params, $value = null)
{
	...
	if(!is_array($params)) {
		if($value === null) {
			s4lox_io_module_log("ms_send_mem: Input string provided, but value missing");
			return;
		}
		// echo "Input is flat\n";
		$params = [ $params => $value ];
