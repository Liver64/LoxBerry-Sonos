...
// Version: SONOS_ACCESS_SOAP_LOG_SEPARATION_V02_2026_06_14
// Logging: SOAP request/response/debug output is written to SOAP-Log-ddmmyyyy.log, not to sonos.log.
...
class SonosAccess
{
    ...
    /**
     * Dedicated SOAP log configuration.
     *
     * IMPORTANT:
     * SOAP request/response/debug output must not be written to sonos.log.
     * It is written to the historical SOAP log file instead:
     *   SOAP-Log-ddmmyyyy.log
     */
    private $soapLogDir = '';
    private $soapLogLevel = 'error';
    private $soapLogEnabled = false;
    ...
    /**
     * Initialize the dedicated SOAP logger.
     *
     * This replaces the old system/Logger.php dependency without mixing verbose
     * SOAP request/response data into the normal Sonos4Lox sonos.log.
     */
    private function initSoapLogger()
    {
        global $lbplogdir;

        if (!empty($lbplogdir)) {
            $this->soapLogDir = rtrim((string)$lbplogdir, '/');
        } elseif (defined('LBPLOGDIR')) {
            $this->soapLogDir = rtrim((string)LBPLOGDIR, '/');
        } elseif (getenv('LBPLOGDIR') !== false) {
            $this->soapLogDir = rtrim((string)getenv('LBPLOGDIR'), '/');
        } else {
            $this->soapLogDir = __DIR__;
        }
        ...
        if (isset($_GET['debug'])) {
            $pluginLevel = '7';
        } elseif (class_exists('LBSystem') && method_exists('LBSystem', 'pluginloglevel')) {
            $pluginLevel = (string)LBSystem::pluginloglevel();
        }

        $this->soapLogLevel = $this->mapPluginLogLevelToSoapLevel($pluginLevel);
        $this->soapLogEnabled = ($this->soapLogLevel !== 'off' && is_dir($this->soapLogDir));
    }

    /**
     * Map the LoxBerry plugin loglevel to the historical SOAP logger levels.
     */
    private function mapPluginLogLevelToSoapLevel($pluginLevel)
    {
        switch ((string)$pluginLevel) {
            case '0':
                return 'off';
            case '3':
                return 'error';
    ...
    /**
     * Write a dedicated SOAP log line.
     *
     * Log format intentionally follows the historical SOAP-Log file style,
     * but every message starts with the new relative file context.
     */
    private function logSoap($message, $level = 'debug')
    {
        if (!$this->soapLogEnabled || !$this->shouldWriteSoapLog($level)) {
            return;
        }

        $file = $this->soapLogDir . '/SOAP-Log-' . date('dmY') . '.log';
        $line = date('c') . ' [' . strtoupper((string)$level) . '] : '
              . 'src/Core/Sonos/sonosAccess.php: ' . (string)$message . PHP_EOL;

        @file_put_contents($file, $line, FILE_APPEND | LOCK_EX);
    }

    /**
     * Decide whether a SOAP message should be written for the configured level.
     */
    private function shouldWriteSoapLog($level)
    {
        $levels = [
            'off'     => 0,
            'error'   => 3,
        ...
        $configured = isset($levels[$this->soapLogLevel]) ? $levels[$this->soapLogLevel] : 3;
        $requested = isset($levels[$level]) ? $levels[$level] : 7;
...
public function GetAvailableServicesMap(): array
{
    ...
    private function processSoapCall($path, $uri, $action,  array $parameter = [])
    {
        try {
			...
			$this->logSoap("SOAP request for {$action}: " . ($client->__getLastRequest()), 'debug');
			$this->logSoap("SOAP response for {$action}: " . ($client->__getLastResponse()), 'debug');
			#echo "REQUEST:\n " . htmlentities($client->__getLastRequest()) . "\n";
			#echo "RESPONSE:\n " . htmlentities($client->__getLastResponse()) . "\n";
        } catch (Exception $e) {
            $faultstring = property_exists($e, 'faultstring') ? $e->faultstring : $e->getMessage();
            $faultcode = property_exists($e, 'faultcode') ? $e->faultcode : (string)$e->getCode();
            if (isset($e->detail->UPnPError->errorCode)) {
                $errorCode = $e->detail->UPnPError->errorCode;
				$this->logSoap('Error during SOAP call: ' . $faultstring . ' ' . $faultcode . ' ' . $errorCode . ' (' . $this->resolveErrorCode($path, $errorCode) . ')', 'error');
                throw new Exception('Error during Soap Call: ' . $faultstring . ' ' . $faultcode . ' ' . $errorCode . ' (' . $this->resolveErrorCode($path, $errorCode) . ')');
            } else {
				$this->logSoap('Error during SOAP call: ' . $faultstring . ' ' . $faultcode, 'error');
