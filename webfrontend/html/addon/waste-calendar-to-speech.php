...
function muellkalender() {
	...
	if ($events + 7 == $checklength){
		LOGGING('waste-calendar-to-speech.php: Please remove &events= from your syntax entry in Sonos4lox configuration or enter add the events you are looking for!',3);
		exit;
	}
...
function calendar() {
	...
	if ($checkdebug == "debug") {
		LOGGING('waste-calendar-to-speech.php: Please remove &debug from your syntax in Sonos4lox Calendar URL!',3);
		exit;
	}
	echo '<PRE><br>';
	...
	if (empty($calendar))  {
		LOGGING('waste-calendar-to-speech.php: No actual appointments according to your "fwday" parameter received or something went wrong!',4);
		LOGGING('waste-calendar-to-speech.php: in addition please check/maintain events to be announced in your calendar URL!',4);
		exit(1);
	}
	#$findMich1 = "&events=";
	#$findMich2 = "&debug";
	...
	echo ($speak);
	#echo '<br><br>';
	LOGGING('waste-calendar-to-speech.php: Calendar Announcement: '.$speak,7);
	LOGGING('waste-calendar-to-speech.php: Message been generated and pushed to T2S creation',5);
	return $speak;
	}
}
