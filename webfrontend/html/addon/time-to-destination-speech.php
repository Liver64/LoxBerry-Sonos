...
function tt2t()
{
	...
	if (empty($_GET['to'])) {
		LOGGING('time-to-destination-speech.php: You do not have a destination address maintained in syntax. Please enter address!',3);
		exit;
    } else {
		$arrival = $_GET['to'];
		LOGGING('time-to-destination-speech.php: Valid destination address has been found!',5);
	}
	...
	if (!isset($_GET['model'])) {
		...
		if (in_array($traffic_model, $valid_traffic_models)) {
			$traffic_model 	= $_GET['model'];
			LOGGING('time-to-destination-speech.php: Valid traffic model has been entered!',5);
		} else {
			LOGGING('time-to-destination-speech.php: The traffic model you have entered is invalid. Please correct!',3);
			exit;
		}
	}
	...
	if (isset($_GET['deptime']))  {
		...
		if ($deptime === false)  {
			LOGGING('time-to-destination-speech.php: Something went wrong with your time entry, please correct! Only 24h or US/UK formats are allowed (eg. 14:30 or 2:30pm)',3);
			exit(1);
		}
		if ($deptime < time())  {
			LOGGING('time-to-destination-speech.php: The departure time from your syntax need to be a future time! Current time you have entered is in the past, please correct',3);
			exit(1);
		} else {
			$time = $deptime;
		}
	...
	if (empty($data)) {
		LOGGING('time-to-destination-speech.php: Data from Google Maps could not be obtainend! Please check your syntax',3);
		exit;
	} else {
		LOGGING('time-to-destination-speech.php: Data from Google Maps has been successful obtainend.',6);
	}
    ...
    if ($status == "OK" && $row_status == "OK") {
		...
		LOGGING('time-to-destination-speech.php: The entered URL is not complete or invalid. Please check URL!',3);
        exit;
    }
    $words = $text;
	#echo $request;
	...
	#echo $text;
	LOGGING('time-to-destination-speech.php: Destination announcement: '.($ttd),7);
	LOGGING('time-to-destination-speech.php: Message been generated and pushed to T2S creation',5);
	return $words;
}
