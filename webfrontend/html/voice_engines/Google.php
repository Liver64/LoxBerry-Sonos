...

{
	...
	$valid_languages = File_Get_Array_From_JSON($url, $zip=false);
		...
		if (isset($_GET['lang'])) {
			...
			if (!empty($isvalid)) {
				$language = $_GET['lang'];	
				LOGGING('Sonos: voice_engines\google.php: T2S language has been successful entered',5);
			} else {
				LOGGING('Sonos: voice_engines\google.php: The entered Google language key is not supported. Please correct (see Wiki)!',3);
				exit;
			}
		} else {
		...
		if (strlen($textstring) > 100) {
            LOGGING("Sonos: voice_engines\google.php: The Google T2S contains more than 100 characters and therefor could not be generated. Please reduce characters to max. 100!",3);
			exit;
        }
		...
		LOGGING("Sonos: voice_engines\google.php: Google has been successful selected", 7);	
		# Übermitteln des strings an Google.com
		$mp3 = file_get_contents("http://translate.google.com/translate_tts?".$inlay);
		file_put_contents($file, $mp3);
		LOGGING('Sonos: voice_engines\google.php: The text has been passed to google engine for MP3 creation',5);
		return ($filename);
}
