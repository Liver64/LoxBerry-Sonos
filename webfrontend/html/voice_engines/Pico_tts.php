	...
	global $config, $pathlanguagefile, $textstring, $filename;
		...
		if (isset($_GET['lang'])) {
			...
			if (!empty($isvalid)) {
				...
				LOGGING('voice_engines\pico.php: T2S language has been successful entered',5);				
			} else {
				LOGGING("voice_engines\pico.php: The entered Pico language key is not supported. Please correct (see Wiki)!",3);
				exit;
			}
		} else {
		...
		LOGGING("voice_engines\pico.php: Pico has been successful selected", 7);	
		...
		try {
			...
			LOGGING("voice_engines\pico.php: The T2S could not be created! Please try again.",4);
		}
		LOGGING('voice_engines\pico.php: The text has been passed to Pico engine for MP3 creation',5);
		return $filename;
}
