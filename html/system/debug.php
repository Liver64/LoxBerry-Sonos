...
function debug($message = "", $loglevel, $raw = 0)
{
	...
	if (intval($config_loglevel) >= intval($loglevel) )
	{
		...
		if ( $loglevel < 4 ) 
		{
		  if ( isset($message) && $message != "" ) notify ( LBPPLUGINDIR, $L['SAVE.MY_NAME'], $message);
		}
...
function check_size_logfile()  {
	...
	if ( $logsize > 5242880 )
	{
		...
		debug("Set Logfile notification: ".LBPPLUGINDIR." ".$L['BASIS.MAIN_TITLE']." => ".$L['ERRORS.ERROR_LOGFILE_TOO_BIG'],7);
